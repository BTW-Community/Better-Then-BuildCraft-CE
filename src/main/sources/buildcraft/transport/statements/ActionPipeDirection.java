...
public class ActionPipeDirection extends BCStatement implements IActionInternal {
    ...
    public ActionPipeDirection(ForgeDirection direction) {
        super(
                "buildcraft:pipe.dir." + direction.name().toLowerCase(Locale.ENGLISH),
                "buildcraft.pipe.dir." + direction.name().toLowerCase(Locale.ENGLISH));
    ...
    public String getDescription() {
        return direction.name().substring(0, 1) + direction.name().substring(1).toLowerCase(Locale.ENGLISH)
                + " Pipe Direction";
    }
