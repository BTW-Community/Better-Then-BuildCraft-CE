...
public class TriggerPipeSignal extends BCStatement implements ITriggerInternal {
    ...
    public TriggerPipeSignal(boolean active, PipeWire color) {
        super(
                "buildcraft:pipe.wire.input." + color.name().toLowerCase(Locale.ENGLISH)
    ...
    public String getDescription() {
        return String.format(
                StringUtils.localize("gate.trigger.pipe.wire." + (active ? "active" : "inactive")),
    ...
    public void registerIcons(IconRegister register) {
        icon = register.registerIcon(
                "buildcrafttransport:triggers/trigger_pipesignal_" + color.name().toLowerCase()
                        + "_"
                        + (active ? "active" : "inactive"));
    }
