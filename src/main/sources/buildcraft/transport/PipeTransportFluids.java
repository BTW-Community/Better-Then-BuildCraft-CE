...
public class PipeTransportFluids extends PipeTransport implements IFluidHandler, IDebuggable {
    ...
    public void updateEntity() {
        ...
        if (networkSyncTracker.markTimeIfDelay(container.getWorldObj())) {
            ...
            if (packet != null) {
                BuildCraftTransport.instance.sendToPlayers(
                        packet,
    ...
    private void moveFluids() {
        if (fluidType != null) {
            ...
            int outputCount = computeCurrentConnectionStatesAndTickFlows(
                    newTimeSlot > 0 && newTimeSlot < travelDelay ? newTimeSlot : 0);
    ...
    static {
        ...
        fluidCapacities.put(PipeFluidsObsidian.class, 4 * BuildCraftTransport.pipeFluidsBaseFlowRate);
