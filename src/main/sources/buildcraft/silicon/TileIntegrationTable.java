...
public class TileIntegrationTable extends TileLaserTableBase implements ISidedInventory {
    ...
    private void setNewActiveRecipe() {
        ItemStack input = getStackInSlot(0);
        if ((input != null && activeRecipe != null && activeRecipe.isValidInput(input))
                || (input == null && activeRecipe == null)) {
            return;
        }
    ...
    private boolean isRoomForOutput(ItemStack output) {
        ...
        if (StackHelper.canStacksMerge(output, existingOutput)
                && output.stackSize + existingOutput.stackSize <= output.getMaxStackSize()) {
            return true;
        }
        return false;
    }
