...
public class TileQuarry extends TileAbstractBuilder
    ...
    public void updateEntity() {
        ...
        if (stage == Stage.BUILDING) {
            ...
            int energyUsed = this.getBattery()
                    .useEnergy(20, (int) Math.ceil(20D + (double) getBattery().getEnergyStored() / 10), false);
    ...
    protected boolean findFrame() {
        for (int i = 2; i < 6; i++) {
            ...
            if (box.contains(xCoord + o.offsetX, yCoord + o.offsetY, zCoord + o.offsetZ)) {
                return worldObj.getBlock(xCoord + o.offsetX, yCoord + o.offsetY, zCoord + o.offsetZ)
                        == BuildCraftBuilders.frameBlock;
            }
        }
    ...
    protected int getNetworkUpdateRange() {
        return DefaultProps.NETWORK_UPDATE_RANGE
                + (int) Math.ceil(Math.sqrt(yCoord * yCoord + box.sizeX() * box.sizeX() + box.sizeZ() * box.sizeZ()));
    }
