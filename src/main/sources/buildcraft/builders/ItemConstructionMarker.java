...
public class ItemConstructionMarker extends ItemBlock {
        ...
        if (nbt.hasKey("x")) {
            ...
            if (tile1 != null && (tile1 instanceof TileArchitect)) {
                ...
                if (tile1 != tile2 && tile2 != null) {
                    if (tile2 instanceof TileArchitect || tile2 instanceof TileConstructionMarker
    ...
    @Override
    public boolean onItemUse(ItemStack marker, EntityPlayer player, World world, int x, int y, int z, int side,
            float par8, float par9, float par10) {

        TileEntity tile = world.getTileEntity(x, y, z);
        NBTTagCompound nbt = NBTUtils.getItemData(marker);

        if (nbt.hasKey("x") && !(tile instanceof TileBuilder || tile instanceof TileArchitect
